...
const VideoPlayer = ({ video, isPlaying, onPlayingChange }) => {
  ...
  // Reset played state when changing videos
  useEffect(() => {
    setPlayed(0);
  }, [video]);
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-purple-100 to-pink-100 p-4">
        <div
          ref={progressBarRef}
          className="h-2 bg-purple-200 rounded-full mb-2 cursor-pointer relative"
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <button onClick={handlePlayPause} className="text-purple-700 hover:text-purple-900 transition-colors">
              {isPlaying ? <Pause size={24} /> : <Play size={24} />}
            </button>
            <button onClick={handleToggleMute} className="text-purple-700 hover:text-purple-900 transition-colors">
              {muted ? <VolumeX size={20} /> : <Volume2 size={20} />}
            </button>

            <div className="hidden sm:flex items-center w-24 relative">
              <div
                ref={volumeBarRef}
                className="h-1 bg-purple-200 rounded-full w-full cursor-pointer"
            ...
            <button onClick={handleSkipForward} className="text-purple-700 hover:text-purple-900 transition-colors">
              <SkipForward size={20} />
            </button>
          </div>
          <button onClick={handleFullscreen} className="text-purple-700 hover:text-purple-900 transition-colors">
            <Maximize size={20} />
          </button>
        </div>
        <div className="mt-4">
